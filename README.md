# BackPedia

A collection of Markdown Files that together create simple and organized "Backend Notes for NodeJS and Python" space for my personal use. It uses general syntax approach for production ready practices for all concepts, starting from scratch to finish.

**[Explore System Design](./System_Design.md)**

## Table of Contents for 

### NodeJS
- [1. Project Setup](./NodeJS/1_Project_Setup.md)
- [2. Database Setup](./NodeJS/2_Database_Setup.md)
- [3. Authentication & Authorization](./NodeJS/3_Authentication_and_Authorization.md)
- [4. Path Aliasing](./NodeJS/4_Path_Aliasing.md)
- [5. Real Time Communication with WebSockets](./NodeJS/5_WebSocket_Setup.md)
- [6. WebRTC Setup for Video Calls](./NodeJS/6_WebRTC_Setup.md)
- [7. Logging](./NodeJS/7_Logging.md)
- [8. REST API Deep Dive](./NodeJS/8_REST_API_Deep_Dive.md)
- [9. Deep Dive into Security](./NodeJS/9_Security_Deep_Dive.md)
  - [Summary of Security Practices](./Security_Summary.md)
- [10. BaaS Introduction](./NodeJS/10_BaaS_Introduction.md)
- [11. Firebase](./NodeJS/11_Firebase.md)
- [12. Supabase](./NodeJS/12_Supabase.md)





